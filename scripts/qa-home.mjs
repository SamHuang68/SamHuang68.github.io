...
try {
  for (const [width, height] of viewports) {
    ...
    if (metrics.cards.length !== 9) failures.push(`${width}px expected 9 portal cards, found ${metrics.cards.length}`);
    if (metrics.cards.filter((card) => card.live).length !== 5 || metrics.cards.filter((card) => card.source).length !== 4) failures.push(`${width}px availability counts are incorrect`);
